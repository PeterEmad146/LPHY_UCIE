...
/// @title UCIe Master LTSSM Top Wrapper
/// @description The central brain of the Logical PHY. Instantiates and orchestrates 
/// all Stage 1, Stage 2, and Stage 3 bring-up and power management state machines.
module lphy_ltssm #(
    ...
    // =========================================================================
    // 1. ADAPTER INTERFACE (RDI & FDI Status)
    // =========================================================================
    ...
    output logic       pl_protocol_vld,   // Protocol validation reflection
    ...
    input  wire        sb_rx_wake,        // Remote wake trigger for RESET
    input  wire        rx_req_active,      input wire rx_rsp_active,
    ...
    input  wire        rx_trainerror_req,  input wire rx_trainerror_resp,
    
    input  wire        rx_retrain_init_req,  input wire rx_retrain_init_resp,
    input  wire        rx_retrain_start_req, input wire rx_retrain_start_resp,
    ...
    // =========================================================================
    input  wire param_done, cal_done, repairclk_done, repairval_done, 
    input  wire reversal_done, repairmb_done,
    input  wire valvref_done, datavref_done, speedidle_done, txselfcal_done, 
    input  wire rxclkcal_done, valtraincenter_done, valtrainvref_done, 
    input  wire datatraincenter1_done, datatrainvref_done, rxdeskew_done, 
    input  wire datatraincenter2_done,
    input  wire linkspeed_done, linkspeed_error, needs_repair, needs_speed_degrade, 
    input  wire repair_done,
    
    input  wire cal_error, is_unrepairable, 
    input  wire internal_retrain_req, internal_error_req,
    ...
    output logic [7:0] error_log_0,       // Real-time LTSSM logging bus
    ...
    // FIXED: Added dedicated pm_exit_error wire to prevent Multiple Drivers!
    logic pm_exit_speedidle, pm_exit_reset, pm_exit_error; 
    
    logic ret_exit_txselfcal, ret_exit_speedidle, ret_exit_repair, ret_exit_error;
    logic err_exit_reset;

    // Sub-module Interconnects for MUXing
    wire [3:0] sbinit_state_sts, mbinit_state_sts, mbtrain_state_sts;
    wire [3:0] linkinit_state_sts, active_state_sts, pm_state_sts, phyret_state_sts;
    
    wire sbinit_inband, mbinit_inband, mbtrain_inband, linkinit_inband, pm_inband;
    wire sbinit_protocol_vld, linkinit_protocol_vld;
    
    wire [7:0] mbinit_log, mbtrain_log, linkinit_log, active_log, pm_log, phyret_log, err_log;
    wire       phyret_stallreq;
    
    wire       linkinit_tx_req_active, linkinit_tx_rsp_active;
    wire       pm_tx_req_active, pm_tx_rsp_active;
    ...
    lphy_ltssm_sbinit #(.TIMEOUT_CYCLES(SBINIT_TIMEOUT)) u_sbinit (
        .clk(clk), .rst_n(rst_n), .en_sbinit(en_sbinit), .package_type(package_type),
        ...
        .pl_state_sts(sbinit_state_sts), .pl_inband_pres(sbinit_inband), .pl_protocol_vld(sbinit_protocol_vld),
        .exit_to_mbinit(sb_exit_mbinit), .exit_to_trainerror(sb_exit_error)
    );

    lphy_ltssm_mbinit #(.TIMEOUT_CYCLES(MBINIT_TIMEOUT)) u_mbinit (
        .clk(clk), .rst_n(rst_n), .en_mbinit(en_mbinit), .package_type(package_type),
        ...
        .pl_state_sts(mbinit_state_sts), .pl_inband_pres(mbinit_inband), .mbinit_substate_log(mbinit_log),
    ...
    lphy_ltssm_mbtrain #(.TIMEOUT_CYCLES(MBTRAIN_TIMEOUT)) u_mbtrain (
        .clk(clk), .rst_n(rst_n), .en_mbtrain(en_mbtrain), .package_type(package_type),
        ...
        .pl_state_sts(mbtrain_state_sts), .pl_inband_pres(mbtrain_inband), .mbtrain_substate_log(mbtrain_log),
    ...
    lphy_ltssm_linkinit #(.TIMEOUT_CYCLES(LINKINIT_TIMEOUT)) u_linkinit (
        .clk(clk), .rst_n(rst_n), .en_linkinit(en_linkinit), .lp_state_req(lp_state_req),
        .rx_req_active(rx_req_active), .rx_rsp_active(rx_rsp_active),
        .tx_req_active(linkinit_tx_req_active), .tx_rsp_active(linkinit_tx_rsp_active),
        .pl_inband_pres(linkinit_inband), .pl_state_sts(linkinit_state_sts), .linkinit_log(linkinit_log),
    ...
    lphy_ltssm_active u_active (
        .clk(clk), .rst_n(rst_n), .en_active(en_active), 
        .lp_state_req(lp_state_req), .lp_linkerror(lp_linkerror),
        .pl_state_sts(active_state_sts), .active_log(active_log),
        ...
        .exit_to_disable(act_exit_disable), .exit_to_retrain(act_exit_retrain), 
        .exit_to_trainerror(act_exit_error)
    );

    lphy_ltssm_pm u_pm (
        .clk(clk), .rst_n(rst_n), .en_l1(en_l1), .en_l2(en_l2),
        .lp_state_req(lp_state_req), .pl_state_sts(pm_state_sts), .pl_inband_pres(pm_inband),
        .rx_req_active(rx_req_active), .rx_rsp_active(rx_rsp_active),
        .tx_req_active(pm_tx_req_active), .tx_rsp_active(pm_tx_rsp_active), .pm_log(pm_log),
        // FIXED: Using independent exit wire for PM to avoid dual-driver conflict
        .exit_to_speedidle(pm_exit_speedidle), .exit_to_reset(pm_exit_reset), 
        .exit_to_trainerror(pm_exit_error) 
    );

    lphy_ltssm_phyretrain #(.TIMEOUT_CYCLES(PHYRETRAIN_TIMEOUT)) u_phyretrain (
        .clk(clk), .rst_n(rst_n), .en_phyretrain(en_phyretrain),
        .local_retrain_trigger(internal_retrain_req), .local_retrain_enc(local_retrain_enc), 
        .pl_stallreq(phyret_stallreq), .lp_stallack(lp_stallack), .pl_state_sts(phyret_state_sts),
        ...
        .tx_retrain_enc(tx_retrain_enc), .phyretrain_log(phyret_log),
    ...
    always_comb begin
        ...
        // Defaults for global buses
        pl_inband_pres  = 1'b0;
        pl_protocol_vld = 1'b0;
        ...
        case (state)
            ...
            ST_SBINIT: begin
                en_sbinit       = 1'b1;
                pl_state_sts    = sbinit_state_sts; 
                pl_inband_pres  = sbinit_inband;
                pl_protocol_vld = sbinit_protocol_vld;
                error_log_0     = 8'h01; 
            ...
            ST_MBINIT: begin
                en_mbinit       = 1'b1;
                pl_state_sts    = mbinit_state_sts; 
                pl_inband_pres  = mbinit_inband;
                error_log_0     = mbinit_log; 
            ...
            ST_MBTRAIN: begin
                en_mbtrain      = 1'b1;
                pl_state_sts    = mbtrain_state_sts;
                pl_inband_pres  = mbtrain_inband;
                error_log_0     = mbtrain_log;
            ...
            ST_LINKINIT: begin
                en_linkinit     = 1'b1;
                pl_state_sts    = linkinit_state_sts;
                pl_inband_pres  = linkinit_inband; 
                error_log_0     = linkinit_log;
            ...
            ST_ACTIVE: begin
                en_active       = 1'b1;
                pl_state_sts    = active_state_sts; 
                pl_inband_pres  = 1'b1;
                pl_protocol_vld = 1'b1; // Hold high during active operation
                error_log_0     = active_log;
            ...
            ST_L1: begin
                en_l1           = 1'b1;
                pl_state_sts    = pm_state_sts; 
                pl_inband_pres  = pm_inband;
                error_log_0     = pm_log;
                // FIXED: Route PM error triggers to TrainError
                if (pm_exit_speedidle) next_state = ST_MBTRAIN;
                else if (pm_exit_error) next_state = ST_TRAINERROR;
            end
            ...
            ST_PHYRETRAIN: begin
                en_phyretrain   = 1'b1;
                pl_state_sts    = phyret_state_sts;
                pl_inband_pres  = 1'b1;
                pl_stallreq     = phyret_stallreq;
                error_log_0     = phyret_log;
