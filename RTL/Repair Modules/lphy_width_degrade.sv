...
/// @title UCIe Width Degradation Multiplexer
/// @description Spatially routes logical active lanes to physical surviving lanes 
/// when the package suffers half-width fatal damage. Parks unused lanes at 0.
module lphy_width_degrade #(
    parameter int NUM_LANES = 64
)(
    // Control signal from Data Repair Controller / LTSSM
    input  wire  [1:0] lane_map, // 2'b11: Full, 2'b01: Lower Half, 2'b10: Upper Half, 2'b00: Fail
    
    // TX Data Path (8-bit Parallel Bytes)
    input  wire  [7:0] tx_logical_data [NUM_LANES-1:0], 
    output logic [7:0] tx_physical_data [NUM_LANES-1:0], 
    
    // RX Data Path (8-bit Parallel Bytes)
    input  wire  [7:0] rx_physical_data [NUM_LANES-1:0], 
